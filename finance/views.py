...
def _expense_queryset(request):
    ...
    items = filter_selected_cycle(request, OperationalExpense.objects.select_related('pond').prefetch_related('documents').order_by('-date', '-created_at', '-id'))
    ...
    # Re-apply the ordering after every filter so pagination can never
    # expose an accidental/default database order.
    items = items.order_by('-date', '-created_at', '-id')
