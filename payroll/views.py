...
def _sync_expense(record):
    # OperationalExpense hanya dibuat untuk gaji yang benar-benar sudah dibayar.
    # Jika status payroll berubah dari paid menjadi unpaid/partial, hapus expense
    # otomatis yang sebelumnya ditautkan agar laporan basis kas tidak stale.
    if record.payment_status != 'paid' or not record.payment_date:
        if record.expense_id:
            record.expense.delete()
            PayrollRecord.objects.filter(pk=record.pk).update(expense=None)
            record.expense = None
        return
    defaults = {
