...
ROLE_PERMISSIONS = {
    ...
    'akuntan': {
        'dashboard', 'ai.finance', 'ai.payroll',
 'sales.dashboard', 'sales.invoices', 'finance.expenses', 'finance.profit_loss', 'finance.periodic_report', 'finance.tax_reports', 'payroll.view', 'payroll.manage'
    },
    'investor': {
...
def visible_menu(user):
    ...
    for item in MENU_DEFINITIONS:
        ...
        if item.get('owner_only') and not is_owner(user):
            continue
        # AI Employee Finance & Payroll is accessed contextually from
        # Pengeluaran Operasional, not exposed as a standalone sidebar menu.
        if str(item.get('label', '')).strip().lower() in {
            'ai employee finance & payroll', 'ai employee',
        }:
            continue
