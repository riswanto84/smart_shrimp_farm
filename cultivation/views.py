...
def cycle_report_pdf(request, pk):
    """Laporan akhir siklus lengkap: produksi, seluruh operasional, penjualan, dan keuangan."""
    from .comprehensive_report import generate_comprehensive_cycle_pdf
    return generate_comprehensive_cycle_pdf(request, pk)
...
def cycle_history_excel(request, pk):
    ...
    # Format angka Excel Indonesia: pemisah ribuan dan desimal konsisten.
    for row in range(5, ws.max_row + 1):
        label = str(ws.cell(row, 1).value or '').lower()
        cell = ws.cell(row, 2)
        if isinstance(cell.value, (int, float)):
            if any(k in label for k in ('tebar',)):
                cell.number_format = '#,##0'
            elif any(k in label for k in ('sr', 'roi', 'fcr', 'adg', 'abw')):
                cell.number_format = '#,##0.00'
            else:
                cell.number_format = '#,##0.00'
    integer_cols = {2, 5, 7}
    decimal2_cols = {4, 8, 9, 11, 12, 13, 14}
    decimal3_cols = {10}
    for row in range(2, ws2.max_row + 1):
        for col in integer_cols:
            ws2.cell(row, col).number_format = '#,##0'
        for col in decimal2_cols:
            ws2.cell(row, col).number_format = '#,##0.00'
        for col in decimal3_cols:
            ws2.cell(row, col).number_format = '#,##0.000'
