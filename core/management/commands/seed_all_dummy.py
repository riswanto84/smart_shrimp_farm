        ...
        for i in range(count):
            ...
            DailyParameter.objects.update_or_create(
                ...
                defaults={
                    ...
                    'transparency_morning': Decimal('30.00') + Decimal(i % 16),
                    'transparency_evening': Decimal('28.00') + Decimal(i % 14),
                    'feed_kg': Decimal('25.00') + Decimal(i % 35),
                    'mortality': (i * 3) % 120,
                    'water_color': colors[i % len(colors)],
                    'water_color_morning': colors[i % len(colors)],
                    'water_color_evening': colors[(i + 1) % len(colors)],
